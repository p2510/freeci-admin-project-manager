...
let getData = (link) => {
  axios
    .get(link || apiRoutes['contract.index'])
...
const filteredContract = computed(() => {
  return contracts.value?.data.filter((item) => {
    return item.id == contractId.value
  })
})

let confirmed = (id) => {
  axios
    .patch(apiRoutes['contract.confirmed'] + `/${id}`)
    .then(function (response) {
...
let deleteContract = (title) => {
  loadingBtn.value = true
  showActionModal.value = false
  axios
    .delete(apiRoutes['contract.destroy'] + `/${title}`)
    .then(function (response) {
      loadingBtn.value = false
      getData()
    })
...
let changeStatut = (id) => {
  ...
  showActionModal.value = false
  axios
    .patch(apiRoutes['contract.changestatut'] + `/${id}`)
    .then(function (response) {
      loadingBtn.value = false
      getData()
    })
...
<template>
  <main class="w-full mx-0 p-4 h-screen">
    ...
    <div class="relative overflow-auto w-full bg-white dark:bg-slate-800 mt-6 rounded-lg h-4/5">
      ...
      <div v-else>
        <NothingResource v-if="contracts.length == 0" />
        ...
        >
          ...
          >
            <tr>
              ...
              <th scope="col" class="px-6 py-3 font-semibold text-start">Catégorie</th>
          ...
          <tbody>
            ...
            >
              ...
              <td class="px-6 py-4 font-medium text-gray-900 dark:text-white">
                <span v-if="contract.statut"> Ouvert</span>
                <span v-else> Fermé </span>
              </td>

              <td class="px-6 py-4 font-medium text-gray-900 dark:text-white">
                <p
                  v-if="contract.authorize"
                  class="text-center text-white bg-green-700/80 rounded-full"
                >
                  OK
                </p>
                <SecondaryBtn v-else @click="confirmed(contract.id)"> Authoriser </SecondaryBtn>
              </td>

              <td class="px-6 py-4 gap-4 flex justify-start items-center">
                <PrimaryBtn @click="showActionModalFunction(contract.id)"> Gérer </PrimaryBtn>
              </td>
            </tr>
          </tbody>
        </table>
    ...
    <!-- paginate -->
    <div v-if="contracts.length !== 0" class="mt-4 flex justify-center">
        ...
        >
          ...
          >
            <div class="bg-white px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
              <!-- Contenu de la modale -->
              <h3 class="text-lg font-medium text-gray-900" id="modal-title">Gestion de contrat</h3>
              <div class="mt-5 sm:mt-4" v-for="item in filteredContract" :key="item.id">
                <div class="space-y-4 w-full p-4">
                  <p class="font-semibold text-lg">
                    <img
                      class="w-40 h-30 object-cover"
                      :src="baseUrl + '/storage/logo_entreprise/' + item.logo"
                      alt=""
                    />
                  </p>
                  ...
                  <p class="text-md">Salaire Max : {{ item.budget_max }} F</p>
                  <p class="text-md">Salaire Min : {{ item.budget_min }} F</p>
                ...
                <PrimaryBtn
                  ...
                  @click="deleteContract(item.title)"
                  >Supprimer le contrat</PrimaryBtn
                >
                <SecondaryBtn
                  v-if="item.statut"
